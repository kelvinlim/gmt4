...
class Measure {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Measure &&
     other.start == start &&
     other.end == end &&
  ...
  @override
  String toString() => 'Measure[start=$start, end=$end, ordinal=$ordinal, json=$json, complete=$complete, lock=$lock, measureTypeId=$measureTypeId, measureType=$measureType, sessionId=$sessionId, session=$session, id=$id, deleted=$deleted, createdDate=$createdDate]';
  ...
  static List<Measure>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Measure>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Measure>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Measure.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
